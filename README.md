...
- **결론**
  ...
  - 향후 마케팅 전략 수립 시 검색량 데이터를 이용해 관중수 유입률 증가를 극대화시킬 수 있는 방안을 고려할 예정입니다.
...
- 종목별 선호도:
  - 야구의 경우 모든 지역에서 가장 인기있으며 축구의 경우 서울과 경기지역에서 높은 선호도를 보입니다.
  - 농구와 배구의 경우 관중수는 낮지만 특정 지역에서 관심도를 보이고 있습니다.
- 마케팅 인사이트:
  - 야구: 전국적인 이벤트 및 구장 내 경험을 강화한다면 더 많은 관중수를 이끌어 낼 수 있을 것이라 판단됩니다.
  - 축구: 서울과 경기권 중심으로 대규모 프로모션을 기획하는 것이 좋을 것이라 판단됩니다.
...
- 네 종류의 모델 중 비선형 데이터 특화인 랜덤포레스트와 XGBoost의 성능값이 우수하게 도출되었는데 그 중 랜덤포레스트의 성능값이 더 우수하여 해당 알고리즘을 채택하게 되었습니다.
...
- 4개 종목 동일하게 seasonal_order = (1, 1, 1, 60) 해당 값으로 모델 학습을 진행하였습니다.
