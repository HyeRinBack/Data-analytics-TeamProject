...
---
## 6. 머신러닝
머신러닝 모델을 활용하여 **2025년 국내 프로 스포츠 직관 관중수 예측 및 스포츠 선호도 변화 추이**를 예측했습니다.
이를 통해 **연령대별 선호도 변화 및 종목별 관중수 증감 추세**를 파악하여 미래 지향적 **마케팅 전략을 수립**하고자 했습니다.

### 6.1 개요
#### 목표:
- **2025년 관중수 예측**:
  - **경기장별 관중수 데이터**를 기반으로 **2025년 관중수 변화**를 예측합니다.
- **종목별 온라인 검색량 예측**:
  - **검색량 데이터**를 활용하여 **스포츠 선호도 변화**를 예측합니다.
#### 모델 선정 이유:
- **관중수 데이터**: **비선형적 패턴**이 강하기 때문에 **비선형 회귀 모델**을 적용하였습니다.
- **검색량 데이터**: **계절성과 트렌드 패턴**이 존재하여 **시계열 모델**을 적용하였습니다.

### 6.2 경기장별 관중수 예측
#### 6.2.1 데이터 전처리 및 특징 선택
- 타겟 변수: **관중수**
- 독립 변수:
  - **경기장 위치(지역 변수)**: 타겟 인코딩 적용
  - **날짜, 요일 변수**: 원-핫 인코딩
  - **검색량 및 선호도 데이터**: 연속형 변수 스케일링 적용

#### 6.2.2 모델 학습 및 평가
- **회귀 알고리즘** 선택:
  - 선형 회귀 모델: Linear Regression, MLP Regressor
  - 비선형 회귀 모델: Random Forest, XGBoost
- **모델 선정 이유**:
  - **Linear Regression**: 데이터의 **선형 관계** 확인
  - **Random Forest 및 XGBoost**: **비선형 관계** 및 **복잡한 상호작용** 학습에 강점
...
- **모델별 성능 비교**:
  |모델|RMSE|R²|
  |-|-|-|
  |Linear Regression|3248.91|0.7422|
  |Random Forest|2892.38|0.7957|
  |XGBoost|2977.3|0.7835|
  |MLP Regressor|3405.0|0.7168|
 
#### 결과 및 해석:
- Random Forest 모델이 가장 높은 예측 성능을 보여 최종 모델로 선정하였습니다.
- 비선형 관계를 학습하는 랜덤 포레스트가 관중수 데이터의 비선형 패턴을 가장 잘 예측합니다.

#### 6.2.4 모델 예측 및 결과값
#### 예측 결과 개요:
- **GridSearch**를 통한 **Random Forest Regressor 모델**을 사용하여 **2025년 스포츠 경기 관중수 예측**을 진행했습니다.
- **2024년 데이터 기반 검색량과 선호도 점수**를 바탕으로 **2025년 경기일정별 관중수를 예측**했습니다.
- **배구 경기의 2025년 1월 관중수**는 **실제 집계된 관중수**와 **예측값**을 비교하여 모델의 **정확도를 검증**했습니다.
#### 표: 2024년 데이터에 따른 관중수 예측값 vs 2025년 실제 집계 관중수(배구)

|장소|종목|월|일|지역|2024년 데이터에 따른 [관중수 예측값]|2025년 실제 관중수|오차|
|-|-|-|-|-|-|-|-|
|천안유관순체육관|배구|1|7|충청남도|2175|2319|144|
|인천삼산월드체육관|배구|1|7|인천광역시|2496|2838|342|
|김천실내체육관|배구|1|9|경상북도|1245|1173|-932|
|의정부체육관|배구|1|10|경기도|1831|1357|-820|
|천안유관순체육관|배구|1|6|충청남도|2215|1855|-360|
|천안유관순체육관|배구|1|7|충청남도|1863|2484|621|
|인천삼산월드체육관|배구|1|10|인천광역시|2642|1861|-781|
|인천삼산월드체육관|배구|1|11|인천광역시|4071|4812|741|
|인천삼산월드체육관|배구|1|15|인천광역시|2434|1536|-898|
|김천실내체육관|배구|1|15|경상북도|2125|1844|-281|
|인천삼산월드체육관|배구|1|12|인천광역시|2008|1269|-739|
|인천삼산월드체육관|배구|1|6|인천광역시|2692|1831|-861|
|인천삼산월드체육관|배구|1|15|인천광역시|3754|3147|-607|
|수원실내체육관|배구|1|11|경기도|4670|2953|-1717|
|안산상록수체육관|배구|1|8|경기도|3202|3027|-175|
|수원실내체육관|배구|1|11|경기도|4670|2953|-1717|
|천안유관순체육관|배구|1|7|충청남도|2175|2319|144|
|화성체육관|배구|1|24|경기도|1906|1181|-725|

#### 인사이트 및 분석:
- **2024년 데이터 기반 예측값**과 **2025년 실제 관중수** 간의 **오차가 발생한 원인은** 다음과 같습니다.
  - **경기 중요도 및 이벤트성 변수**: 예측 모델이 경기 중요도나 특별 이벤트를 반영하지 못한 경우 오차가 발생했습니다.
  - **날씨 및 외부 요인**: 야외 경기장의 경우 **날씨나 교통 상황**등이 관중수에 영향을 미쳤을 가능성이 있습니다.
  - **데이터 불균형 문제**: 특정 경기장이나 특정 날짜에 **데이터가 부족하거나 불균형**하여 예측 정확도가 떨어졌을 수 있습니다.

#### 한계점 및 개선 방안:
- 한계점:
  - **모델의 한계**: **비선형 회귀 모델(Random Forest)이** 일부 외부 변수를 반영하지 못하여 **예측 오차가 발**생했습니다.
  - **데이터 부족**: **농구와 배구**는 **데이터가 부족**하여 일부 경기에서 오차가 크게 발생했습니다.
- 개선 방안:
  - 데이터 추가 수집: **이벤트 변수, 날씨 데이터, 경기 중요도** 등을 **추가**하여 모델의 외부 요인 반영
 - 모델 고도화: XGBoost와 같은 **복합 모델을 사용**하여 정확도 향상
 - **실시간 데이터 업데이트**: 실시간 검색량 데이터와 연동하여 **예측 정확도**를 높입니다.

#### 마케팅 전략 및 실무 적용:
- **예측값에 기반한 사전 마케팅** 캠페인
  - 예측 관중수가 높은 경기에 대해 **사전 마케팅 캠페인** 강화
  - 디지털 광고 집행 시기를 **관중수 피크 예상 시점에 맞춰** 효율 극대화
- 예측 오차가 큰 경기 대응 전략
  - 오차가 큰 경기는 **특별 프로모션**을 통해 관중수 유도
    (ex. 현장 할인 이벤트 및 SNS 프로모션)
- 데이터 기반 의사결정
  - 예측 데이터를 기반으로 **경기 일정 및 프로모션 기획**
  - 연간 마케팅 전략 수립에 있어 **트렌드 분석 및 예측 데이터** 활용
 
### 6.3 종목별 온라인 검색량 예측
#### 6.3.1 데이터 전처리 및 특징 선택
- 타겟 변수: **온라인 검색량**
- 독립 변수:
  - **날짜, 요일 변수**: 계절성과 트렌드 반영을 위해 요일 및 월 변수 사용
  - **선호도 데이터**: 연령대별 및 성별 선호도 점수 추가
  - **경기 일정 변수**: 경기일 전후 검색량 변화를 반영하기 위해 경기 일정 데이터 추가
- 정상성 검정 및 차분:
  - ADF Test를 통해 정상성 검정 실시
  - 정상성을 만족하지 않는 데이터는 차분 후 정상성 확보
  - 계절성 패턴 확인: 월별, 분기별 패턴 파악 후 계절성 요일 반영을 위해 SARIMA 모델 적용

#### 6.3.2 모델 선정 및 학습
- 시계열 모델 선택:
  - **ARIMA**
    - 트렌드와 계절성이 없는 데이터의 일반적인 시계열 예측에 적합
  - **SARIMA**
    - 계절성 및 트렌드를 반영하여 예측 정확도를 높일 수 있음
    - **야구, 축구**는 **분기별 계절성이 강하게 나타나** 계절성 패턴 반영을 위해 SARIMA 모델 적용
- 모델 선정 이유:
  - ARIMA: **일반적인 시계열 예측**에 적합
  - SARIMA: **계절성**을 반영한 예측 정확도 향상
- 모델 학습 과정:
  - GridSearchCV를 사용한 **하이퍼파라미터 튜닝**
  - p,d,q 및 P,D,Q,S를 **PACF/ACF 분석**을 통해 결정
  - **교차 검증**을 통해 모델의 일반화 성능 확보
 
#### 6.3.3 모델 성능 평가 및 결과
- **성능 평가 지표**:
  - MAPE(Mean Absolute Percentage Error)- 예측 오차의 백분율
  - RMSE(Root Mean Squared Error) - 예측값과 실제값의 평균 제곱근 오차
- 모델별 성능 비교:
- ARIMA와 SARIMA 모델을 비교한 결과, **SARIMA 모델**이 **계절성 패턴**을 더 정확하게 반영하여 더 나은 성능을 보였습니다.
|종목|모델|MAPE|RMSE|
|-|-|-|-|
|야구|SARIMA|80.82%|32.55|
|축구|SARIMA|99.19%|10.76|
|농구|SARIMA|78.05%|2.75|
|배구|SARIMA|454.87%|5.08|
...
- 한계점 개선 방안:
  - 외부 요인 데이터 추가:
    - 날씨 데이터, 경기 중요도 변수 추가로 예측 정확도 향상
  - 소셜 미디어 데이터 연계:
    - 소셜 미디어 데이터와 검색량 데이터의 상관관계 분석

## 7. 프로젝트 마무리 및 결론
### 7.1 프로젝트 요약 및 성과
- 2024년 데이터를 기반으로 **2025년 스포츠 직관 관중수 예측** 및 **연령대별 스포츠 선호도 변화**를 분석했습니다.
- 정확도 높은 **머신러닝 모델**을 통해 **미래 지향적 마케팅 전략 수립**의 기초 데이터를 확보했습니다.
- **Z세대 타겟팅**과 **지역별 맞춤 마케팅 전략**을 수립하여 스포츠 산업의 성장 가능성을 확인했습니다.

### 7.2 한계점 및 향후 연구 방향
- **더 많은 데이터 축적** 및 **실시간 데이터 연동**으로 **예측 정확도**를 높일 필요가 있습니다.
- 소셜 미디어 데이터와의 **연계 분석**을 통해 **디지털 마케팅 전략**을 더 정교하게 수립할 수 있습니다.

### 최종 결론 및 제안
- 스포츠 산업의 트렌드 변화에 민감하게 대응하기 위해서는 **데이터 기반 의사결정**이 필수적입니다.
- 스포츠 팬 맞춤형 마케팅 전략을 통해 **충성도 높은 팬층 확보 및 지속적인 관중수 증가**를 달성할 수 있습니다.  
